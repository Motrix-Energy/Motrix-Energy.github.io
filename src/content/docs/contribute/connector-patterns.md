...
### 3. The schema-lockstep test follows `**kwargs`, so declare what you accept

**Symptom.** Your subclass inherits an option from its parent — `host`, say — and your schema declares it, since operators do configure it. The lockstep check fails: *"schema allows options the constructor rejects"*, or its mirror, *"constructor takes options the schema does not declare"*.

**Mechanism.** The kwargs contract says config options are spread into the constructor, so the check asserts that schema keys and constructor parameters agree — in **both** directions, since an option the constructor accepts but the schema omits cannot be configured either. It reads the signature and, when it finds `**kwargs`, walks up the MRO to the first `__init__` that does not forward, which is where the real signature begins. So `**kwargs` no longer stops the check dead: it hands it the parent's parameters instead, and your schema is then measured against those too. (That walk is what lets `algorithms/` ship schemas at all — both shipped algorithms are pure `**kwargs` forwarders.)

**The rule.** Re-declare in your own signature the inherited options you want configurable, and forward them to `super()`. Forwarding blindly with `**kwargs` now passes the check, but it leaves your signature silent about what you actually accept — and your schema still has to declare every option the chain takes, so you have moved the bookkeeping rather than avoided it. [`OpenemsConnector.__init__`](https://github.com/Motrix-Energy/motrix-edge/blob/main/connectors/openems.py) does exactly this — it re-declares the parent's `default_interval`, `timeout` and `verify_ssl` beside its own options and passes them through — and its docstring names this rule as the reason.
